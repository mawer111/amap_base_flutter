...
@implementation CalculateWalkRoute {
  RoutePlanParam *_routePlanParam;
  AMapSearchAPI *_search;
  FlutterResult _result;
}
...
- (void)onMethodCall:(FlutterMethodCall *)call :(FlutterResult)result {
  ...
  // 路线请求参数构造
  AMapWalkingRouteSearchRequest *routeQuery = [[AMapWalkingRouteSearchRequest alloc] init];
  routeQuery.origin = _routePlanParam.from;
  routeQuery.destination = _routePlanParam.to;

  [_search AMapWalkingRouteSearch:routeQuery];
}
...
- (void)onRouteSearchDone:(AMapRouteSearchBaseRequest *)request response:(AMapRouteSearchResponse *)response {
  ...
  _result([[[UnifiedWalkRouteResult alloc] initWithAMapRouteSearchResponse:response] mj_JSONString]);
}
